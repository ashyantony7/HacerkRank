...
int libraryFine(int d1, int m1, int y1, int d2, int m2, int y2) {
    ...
    if (y1 > y2) return 10000; // next year

    else if (y1 < y2) return 0; // year early
    
    else{
        if (m1 > m2)  return (m1-m2)*500; // month late
        
        else if (m1 < m2) return 0; // month early
        
        else{
            if (d1 > d2) return (d1-d2)*15; // day late
            else
                return 0;
        }
    }
